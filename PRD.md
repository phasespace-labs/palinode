...
- File watcher daemon (`watchdog`) monitors `~/.palinode/`
...
- **Explicit:** "Remember: Alice prefers single-page checkout" → classified and filed immediately
- **Channels:** Telegram, Slack, webchat, CLI — all go through OpenClaw → Palinode plugin

**Mode 2: Document ingestion (file drops)**

- Watch folder: `~/Palinode-Inbox/` on MBP, synced via Syncthing to `~/.palinode/inbox/raw/`
...
- "You last worked on your project two days ago. Status: integration testing complete, waiting on team feedback."
...
When you spend 30 minutes writing a prompt to an agent — "build a technical spec with role-based access control, team-sensitive" — that prompt is the specification. The output is the compiled artifact. Losing the prompt means losing the intent, constraints, and reasoning.
...
The milestone execution prompt pattern is already this practice — generalized across all work.
...
| **4: Multi-Agent + MCP** | Governor/gradient/emission read access + MCP server for external tools | Future |
