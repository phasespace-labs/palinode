...
- *Example:* "Don't inject raw config files into agent prompts — degraded output quality from 76% to 5%." → Insight (project-specific).
...
- *NOT example:* A user opened vim once in a session → don't infer "prefers vim." Single instances aren't preferences.
...
- *Example:* "Level 3 identity: rich narrative identity where speech patterns are implied, no instructions needed. YAML=DNA, AI=Life." → Decision about character design philosophy.
...
- "You've mentioned context window limitations across Project A, Palinode, and other work — this is a pattern"
