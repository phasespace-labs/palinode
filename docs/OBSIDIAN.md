...
Given a `[[wikilink]]` whose target file does not exist, returns existing files semantically near the link target text. The model suggests a redirect to one of those files, or creates the target with the candidate's content as scaffolding rather than starting cold.
