...
All 22 operations were validated before being applied by `palinode/consolidation/executor.py`. The model never touches the file directly; it emits JSON like `{"op": "SUPERSEDE", "id": "f-0317-1", "superseded_by": "f-0324-2", "reason": "stripe integration actually shipped on the 24th"}`, and Palinode applies only valid changes.
