...
experienced colleague would remember. Five kinds of note:
...
- `form_schema` — the inventory of one form the agent saw: every field label, its
  type, whether it is required, its default value, plus the form's sections and
  buttons. One note per distinct form (a "New Incident" form seen on three
  states is one note; the "Incident" and "Private Task" forms are two). This is
  the only kind that is a structure, not a sentence: put the inventory in
  `fields`, `sections` and `buttons`, and use `content` for one sentence naming
  the form and how you reached it. `required` is true only when the page marks
  it (an asterisk, "required", "Mandatory"); `default` is the pre-filled value
  (a checkbox's checked state, a choice's selected option, "5" in Priority) or
  null when the field was empty. Never infer either.
...
- Up to 25 notes. Each note's `content` is one to three sentences; a
  `form_schema` note's `fields` list is as long as the form.
...
```json
{
  "kind": "fact | transition | procedure | gotcha | form_schema",
...
A `form_schema` item additionally carries the inventory:

```json
{
  "kind": "form_schema",
  "title": "Form schema: Private Task",
  "content": "The \"Private Task\" form, reached from All > Private Tasks > New.",
  "form": "Private Task",
  "sections": ["Details", "Notes"],
  "fields": [
    {"label": "Short description", "type": "text", "required": true, "default": null},
    {"label": "Priority", "type": "choice", "required": false, "default": "5 - Planning"},
    {"label": "Active", "type": "checkbox", "required": false, "default": true}
  ],
  "buttons": ["Submit", "Save", "Delete"],
  "page": "/nav_to.do?uri=vtb_task.do",
  "states": [4, 5],
  "confidence": 0.85
}
```
